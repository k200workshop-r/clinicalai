...
const BUILD = "2026-06-19n";

/* ---------- 教師 token（無狀態簽章，伺服器重啟仍有效，7 天效期）---------- */
const SECRET = process.env.TEACHER_SECRET || ("clinic-sim:" + TEACHER_PASSCODE);
function newToken() {
  const body = Buffer.from(JSON.stringify({ t: "teacher", exp: Date.now() + 1000 * 60 * 60 * 24 * 7 })).toString("base64url");
